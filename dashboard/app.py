...
#!/usr/bin/env python3
"""
dashboard/app.py — HITL Operator Dashboard Server (Tool 4)

This Flask application is the Human-in-the-Loop web interface for Tool 4.
It is the bridge between the anomaly detection pipeline and the operator:

  detect.py  →  hitl.py (AlertQueue)  →  app.py (REST API)  →  dashboard UI
                                                ↓ operator approves
                                          mitigator.py  →  OVS switch

The server runs on port 5000 (separate from Ryu's port 8080) so both
...
# How often (seconds) the background scanner re-runs detect() on live data
...
def create_app(
    ...
    -------
    Flask
        Configured app instance. Call app.run() or use gunicorn to serve.
    """
    app = Flask(
        __name__,
    ...
    def index():
        ...
        if not os.path.exists(index_path):
            # Friendly error if the template hasn't been created yet
            return (
    ...
    def get_mitigation_log():
        ...
        Query parameters:
          ?lines=N   — return the last N lines (default: 50)
        """
    ...
    def verify_mitigation():
        ...
        Query parameters:
          ?dpid=N   — switch DPID to query (default: 1 = s1)

        Returns the raw ovs-ofctl output lines or an error message if
        ovs-ofctl is not available (i.e. not running inside the Mininet VM).
        """
    ...
    def unblock_host():
        """
        Manually unblock a host — remove the Tool 4 DROP rule.
    ...
    def trigger_scan():
        ...
        Useful for the demo video — press a button in the dashboard
        to force a fresh scan without waiting for the auto-scan timer.
    ...
    def health_check():
        """
        Health check endpoint — used by the dashboard to confirm the
...
# ──────────────────────────────────────────────────────────────────────────────
# CLI entry point (also called by cli.py's cmd_dashboard)
# ──────────────────────────────────────────────────────────────────────────────
