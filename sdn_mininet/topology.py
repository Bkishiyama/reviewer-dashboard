...
- External attack mode (Kali -> IoTGoat through Mininet):
  - Kali (192.168.100.3) attacks IoTGoat (192.168.100.2) via br-iot bridge,
    which is patched directly into s3 (dpid=3) via patch-to-iot/patch-to-s3.
  - Traffic routes through s3 so Ryu sees and records it in live_client3.csv
    (alongside h5/h6 traffic, since both share s3).
  - Dashboard detects and blocks Kali's attack.
  - br-iot setup is handled outside this script (setup_iot_bridge.sh); this
    topology no longer creates any Mininet-side gateway host for it.
...
IoTGoat/Kali traffic enters via br-iot's patch ports straight into s3
(dpid=3); this is set up outside this script and does not require any
additional Mininet host or link.
"""
